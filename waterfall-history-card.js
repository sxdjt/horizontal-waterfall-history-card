class WaterfallHistoryCard extends HTMLElement {
  ...
  renderCard(historyData, currentEntity) {
    ...
    const minVal = this.config.min_value ?? Math.min(...processedData.filter(v => v !== null));
    const maxVal = this.config.max_value ?? Math.max(...processedData.filter(v => v !== null));
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div class="card-header">
        ...
        ${this.config.show_current ? `<span class="current-value">${current}${this.config.unit}</span>` : ''}
      </div>

...
@customElement('waterfall-history-card-beta')
export class WaterfallHistoryCardBeta extends LitElement {
  ...
  public static async getConfigElement(): Promise<LovelaceCardEditor> {
    await import('./editor-beta');
    return document.createElement('waterfall-history-card-editor-beta') as LovelaceCardEditor;
  }
...
declare global {
  interface HTMLElementTagNameMap {
    'waterfall-history-card-beta': WaterfallHistoryCardBeta;
  }
}
...
(window as any).customCards.push({
  ...
  description: 'A horizontal waterfall display for historical sensor data with multi-state interval tracking (BETA)'
});

console.info(
  `%c WATERFALL-HISTORY-CARD-BETA %c v4.1.0-beta `,
